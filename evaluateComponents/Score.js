...
const Score = ({setToggleState}) => {
  const [state, setState] = useContext(Context)
    ...
    const non_academic_result = ECactivities + volunteerActivities + achievements + volunteerAchievements;
    
    const sscScore = only_ssc_result + non_academic_result
    const hscScore = converted_ssc_result + converted_hsc_result + non_academic_result
    const diplomaScore = converted_diploma_ssc + converted_diploma_result + non_academic_result
    const bachelorsScore = converted_bachelors_ssc + converted_bachelors_hsc + converted_bachelors_result + non_academic_result
  ...
  function getPrograms(study_level) {
    ...
    return (
        <div> 
           ...
           {(!study_level || !ECactivities || !volunteerActivities || !achievements || !volunteerAchievements)? 
           <Typography style={{padding:'20px 10px',color:'black',fontWeight:'bold'}} variant='h6' align='center'>Please complete all the steps to generate your score.
           </Typography>   
            
          :<>
          <Typography style={{padding:'20px 10px',color:'black',fontWeight:'bold'}} variant='h6' align='center'>Congratulations! </Typography>
              <div className="scoreContainer">
                
                 <span className='score'>Your Score is {getScore(study_level)} </span>
              </div>
